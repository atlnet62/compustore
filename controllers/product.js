...
export const allProduct = async (request, response) => {
    const query =
        "SELECT product.id AS productID, product.title AS product_name, content, image_name, qtyInStock, price, category.title AS category_name FROM product JOIN category ON category_id = category.id";
    ...
    if (request.params.mode === "edit") {
        if (request.params.productID) {
            try {
                ...
                const queryCategory =
                    "SELECT category.id AS categoryID, category.title AS titleCAT FROM category";
...
export const addProduct = async (request, response) => {
    const { title, content, image_name, qtyInStock, price, category_id } =
        request.body;
    ...
    try {
        ...
        const query =
            "INSERT INTO product (title, content, image_name, qtyInStock, price, category_id) VALUES (?, ?, ?, ?, ?, ?)";
...
export const editProduct = async (request, response) => {
    if (request.params.productID) {
        ...
        const query =
            "UPDATE product SET title = ?, content = ?, image_name = ?, qtyInStock = ?, price = ?, category_id = ? WHERE id = ? ";
...
export const addImage = async (request, response) => {
    try {
        await request.files.image_name.mv(
            `public/products/${request.files.image_name.name}`
        );
        response.status(200).json({
